...
router.get("/", function(req, res) {
  // req.session.destroy();
  res.render("index", { req: req, title: "Distribeco - Accueil" });
});

router.get("/accueil", function(req, res) {
  if (!req.xhr || req.headers.accept.indexOf("json") < -1) res.redirect("/");
  res.render("accueil", { req: req, title: "Distribeco - Accueil" });
});
...
router.get("/paiement", function(req, res) {
  ...
  Paiement.create(
    { client: req.session.client, commande: req.session.commande },
    function(err, paiement) {
